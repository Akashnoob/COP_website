...
from .models import Note, Product, Cart, Payment, CreditCard, Smartphone
from . import db
import json
import stripe
...
@views.route('/smartphones')
@login_required
def smartphones():
    smartphones = Smartphone.query.all()
    return render_template('smartphones.html', smartphones = smartphones, user = current_user)


@views.route('/smartphone_detail/<int:id>')
def smartphone_detail(id):
    smartphone = Smartphone.query.get(id)
    return render_template('smartphone_detail.html', smartphone=smartphone, user = current_user)
...
def product_detail(id):
    ...
    if not product:
        abort(404)
    if product.id<70:
        smartphone = Smartphone.query.get(product.id)
        return render_template('smartphone_detail.html', user=current_user, smartphone = smartphone)
...
def cart():
    ...
    if not cart or not cart.product_list:
        ...
        return render_template('no_cart.html', user_id = user_id, cart = cart, user = current_user)
...
def add_to_cart():
    ...
    if (cart.product_list!='') and (cart.product_list[0]==','):
        cart.product_list = cart.product_list[1:]
        cart.quantity_list = cart.quantity_list[1:]
    print('print()cart.product_list')
    print(cart.product_list)
    if cart.product_list!='':
        product_ids = [int(x) for x in cart.product_list.split(',')]
    else:
        product_ids = []
